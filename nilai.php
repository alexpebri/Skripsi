					...
					<table class="table">
						...
						<tbody>
							...
							$halaman = 20;
							  ...
							  $result = mysql_query("SELECT * FROM tbl_sample");
							  $total = mysql_num_rows($result);
							  $pages = ceil($total/$halaman);            
							  $query = mysql_query("SELECT * FROM tbl_sample, tbl_karyawan, tbl_aspek, tbl_faktor WHERE tbl_sample.karyawan=tbl_karyawan.id_karyawan AND tbl_sample.faktor=tbl_faktor.id_faktor AND tbl_faktor.aspek=tbl_aspek.id_aspek ORDER BY tbl_sample.karyawan, tbl_aspek.id_aspek ASC LIMIT $mulai, $halaman")or die(mysql_error());
							  $no =$mulai+1;
